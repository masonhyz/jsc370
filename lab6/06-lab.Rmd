...
```{r}
ids_character <- as.character(ids)
```
...
abstracts <- stringr::str_extract(pub_char_list, "<Abstract>[\\s\\S]*?</Abstract>")
...
abstracts <- stringr::str_trim(abstracts)
```
...
12 of them do not have an abstract.

Now, the title

```{r process-titles}
titles <- stringr::str_extract(pub_char_list, "<ArticleTitle>[\\s\\S]*?</ArticleTitle>")
...
0 or them does not have a title.
...
table_with_scroll <- kableExtra::scroll_box(knitr::kable(database), width = "100%", height = "500px")
table_with_scroll
```
